package opts // import "github.com/wanyvic/prizes/opts"
...
var (
	...
	DefaultHTTPPort = 9333 // Default HTTP Port
	// DefaultUnixSocket Path for the unix socket.
	// Docker daemon by default always listens on the default unix socket
	DefaultUnixSocket = "/var/run/prizes.sock"
...
func ParseHost(defaultToTLS, defaultToUnixXDG bool, val string) (string, error) {
	...
	if host == "" {
		if defaultToUnixXDG {
			...
			socket := filepath.Join(runtimeDir, "prizes.sock")
			host = "unix://" + socket
		}
	} else {
		var err error
